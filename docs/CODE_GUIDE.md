...
`world_3d_renderer.gd` owns one transparent `SubViewport`, orthographic camera and lit `World3D`. The 2D nodes remain authoritative for physics, touch, collisions and tests; the renderer mirrors their positions into real 3D models. Ships and bosses use procedurally extruded watertight meshes, asteroids use spatial low-poly meshes, and projectiles/laser segments use emissive spheres and cylinders. Metallic/roughness materials respond to two directional lights, a violet local light and reflected environment light. Explosion dictionaries are presented through a 3D `MultiMesh`, keeping one draw call for dense debris. Actor `_draw()` methods remain as a disabled fallback.

The player shield has no drawn shell, gleam or chromatic rim. Its shader mode applies only the 10× background displacement, while `deflect_shield_projectile()` curves hostile fire tangentially without changing speed or deleting it. Rocks receive the same treatment. Resistance taps use a separate short-lived ring. Live bosses inspect incoming `PlayerWell` destinations and move their 2D hit position sideways/upward, so their 3D hull visibly burns clear rather than appearing behind an event horizon.
