...
The latest visual pass adds spinning opaque event horizons, cause-specific hull destruction, flowing gravity-curved lasers with boss absorption, a visible guard-powered boss shield, rotating resistance thrust, finite exit pulses, and varied distant planets with correctly layered rings. Shields and resistance taps share a bright segmented warp ring, and every gravity ring, pulse, hole, and tether refracts the sky. Saved menu controls tune screen shake, laser brightness, and gravity distortion. See [the code guide](docs/CODE_GUIDE.md) for the render-safety and touch-latency fixes plus the device regression harness.
