...
The latest visual pass adds spinning opaque event horizons, cause-specific hull destruction, flowing gravity-curved lasers with boss absorption, a visible guard-powered boss shield, rotating resistance thrust, finite exit pulses, and varied distant planets with correctly layered rings. The player shield uses exaggerated background distortion with a faint cyan visibility halo; incoming fire and rocks bend around it instead of disappearing against a solid shell. Resistance taps retain their short segmented feedback ring. Player-created black holes use blue-cyan accretion ribbons, while each alien encounter receives a stable randomly selected energy colour. Saved menu controls tune screen shake, laser brightness, and gravity distortion. See [the code guide](docs/CODE_GUIDE.md) for the render-safety and touch-latency fixes plus the device regression harness.

The fleet uses bright beveled armor, raised plates, shaded recesses and moving reflection highlights, all drawn in 2D. Player hardware is identified in cyan while alien hardware uses violet; each boss preserves the same construction at a larger scale with a unique gravity, carrier, or forge silhouette. Asteroids have irregular interlocking stone plates, rounded lighting, mineral highlights and inset craters. New small, medium and large rocks start as ash, magma and cyan ore respectively. Destroyed large rocks split into two medium pieces, and medium rocks split into two small pieces, preserving their parent's material and average momentum. Each destroyed piece awards 25 points; black-hole absorption removes the whole rock without fragments. The active shield uses a deliberately exaggerated tenfold refraction so its protection remains obvious on a phone.

The game remains 2D. The player fighter now uses detailed silver/navy armor and blue fittings drawn from the supplied concept, with baked reflections, tier-scaled hulls, matching firing hardpoints and textured destruction fragments. Its engine light flows into soft blue plasma; alien engines emit violet plasma. Engine burn strengthens closer to black holes and weakens closer to white holes, without changing gravity strength. The [fighter asset and final generation prompt](mobile/assets/ships/README.md) explain how to reskin it. Bosses detect player-well destinations and physically evade them before the core opens.
