...
The latest visual pass adds spinning opaque event horizons, cause-specific hull destruction, flowing gravity-curved lasers with boss absorption, a visible guard-powered boss shield, rotating resistance thrust, finite exit pulses, and varied distant planets with correctly layered rings. The player shield is visible only as exaggerated background distortion; incoming fire and rocks bend around it instead of disappearing against a glowing shell. Resistance taps retain their short segmented feedback ring. Saved menu controls tune screen shake, laser brightness, and gravity distortion. See [the code guide](docs/CODE_GUIDE.md) for the render-safety and touch-latency fixes plus the device regression harness.

The fleet uses a shared angular spacecraft language: layered graphite armor, swept blade silhouettes, recessed weapon ports, luminous energy seams, and bright engine cores. Player hardware is identified in cyan while alien hardware uses violet; each boss preserves the same construction at a larger scale with a unique gravity, carrier, or forge silhouette. Asteroids use irregular directional facets, inset craters, illuminated rims, and damage cracks. The active shield uses a deliberately exaggerated tenfold refraction so its protection remains obvious on a phone.

Gameplay remains a 2D portrait shooter, while `world_3d_renderer.gd` mirrors its coordinates into a transparent orthographic Godot 3D world. Ships and bosses use extruded watertight meshes with metallic/roughness materials; asteroids use low-poly spatial meshes; plasma, rockets and lasers use emissive 3D geometry; explosions use a 3D debris multimesh. Directional and local lights produce real surface reflections and depth without changing the established 2D collision rules. Bosses detect player-well destinations and physically evade them before the core opens.
