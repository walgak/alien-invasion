...
The fleet uses a shared angular spacecraft language: layered graphite armor, swept blade silhouettes, recessed weapon ports, luminous energy seams, and bright engine cores. Player hardware is identified in cyan while alien hardware uses violet; each boss preserves the same construction at a larger scale with a unique gravity, carrier, or forge silhouette. The three asteroid sizes now use distinct mineral treatments: sharp ash ore, dark magma rock with branching molten seams, and cold cyan ore with layered crater bowls. Damage opens brighter fractures through each material. The active shield uses a deliberately exaggerated tenfold refraction so its protection remains obvious on a phone.
