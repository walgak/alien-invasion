...
func run() -> void:
	...
	game.gravity_fields.step(0)
	check(game.lingering_wells == [a] and is_equal_approx(a.remaining, 7.0), "player/boss merger preserves player well and sums 3 plus 4 seconds")
	fresh()
	...
	game._process(game.death_visual.DURATION-0.6+0.01)
