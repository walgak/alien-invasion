...
func run_checks() -> void:
	...
	check(rock.health == 1 and game.asteroids.size() == 1, "the first shot cracks an asteroid")
	game.hit_asteroid(rock)
	check(game.asteroids.is_empty() and game.score == 25, "the second shot destroys an asteroid and scores 25")
